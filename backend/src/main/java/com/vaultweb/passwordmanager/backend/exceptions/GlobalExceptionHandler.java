...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handlePasswordBreachCheckError(
        ...
        log.error("Password Breach check failed: {} - Path: {}", ex.getMessage(), request.getDescription(false));
        return buildErrorResponse("Unable to verify password security at this time. Please try again later",
                HttpStatus.SERVICE_UNAVAILABLE, "BREACH_CHECK_UNAVAILABLE");
    }
    ...
    public ResponseEntity<Map<String, Object>> handleGeneralErrors(Exception ex, WebRequest request) {
        log.error("Unexpected error - Path: {} - Exception: {}",
                ...
                ex);
        return buildErrorResponse("Internal server error", HttpStatus.INTERNAL_SERVER_ERROR,"INTERNAL_ERROR");
    }
}
